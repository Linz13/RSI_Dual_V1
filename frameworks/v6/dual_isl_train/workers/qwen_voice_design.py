...
def main() -> None:
    ...
    if args.action not in {"generate-audio", "prepare-codecs", "sft-update", "preflight"}:
        raise ValueError("V6 TTS only supports generation/codec preparation/SFT/preflight")
    ...
    try:
        ...
        if args.action == "prepare-codecs":
            if distributed.enabled:
                output, extra_metrics = run_sharded_inference(rows, args.output, distributed, worker.prepare_codecs)
                inference_action = True
            else:
