...
MODE="${1:-train}"
RUN_DIR="${2:-${DUALISL_RUN_DIR:-$PROJECT_ROOT/runs/qwen25_omni_3b_reward_v4_10rounds_4gpu_run01}}"
...
CONFIG="$PROJECT_ROOT/configs/train_4gpu_h100_qwen2_5_omni_3b_reward_v4.yaml"
cd "$PROJECT_ROOT"
...
"$DRIVER_PYTHON" -m scripts.qwen25_reward_v4_4gpu_run_guard --config "$CONFIG" --mode train --rounds 10
...
print(json.dumps({'run_dir':config['run']['output_dir'], 'captioner':config['captioner']['model_path'],
                  'world_size':4,'rounds':10,'cycle_sft_selection':config['reward']['cycle_sft_selection'],
...
if [[ -z "${CUDA_VISIBLE_DEVICES:-}" ]]; then echo 'Set CUDA_VISIBLE_DEVICES to the four allocated H100 GPUs.' >&2; exit 2; fi
"$DRIVER_PYTHON" - <<'PY'
import torch
if torch.cuda.device_count() != 4:
    raise ValueError('Exactly four visible H100 GPUs required')
if any('H100' not in torch.cuda.get_device_name(i) for i in range(4)):
    raise ValueError('This entry targets a single node of four H100 GPUs')
PY
mkdir -p "$RUN_DIR"
exec 9>"$RUN_DIR/.launcher.lock"
if ! flock -n 9; then echo 'Another launcher holds this run lock.' >&2; exit 2; fi
ACTION=$("$DRIVER_PYTHON" -m scripts.qwen25_reward_v4_4gpu_run_guard --config "$CONFIG" --mode train --rounds 10 --reserve)
chmod a+rwx "$RUN_DIR"
echo "Qwen2.5-Omni-3B RewardV4 rounds=10 GPUs=4 action=$ACTION run=$RUN_DIR"
"$DRIVER_PYTHON" -m dual_isl_train "$ACTION" --config "$CONFIG"
"$DRIVER_PYTHON" scripts/verify_tts_ddp_sync.py --run-dir "$RUN_DIR" --expected-world-size 4 --require-memory-bounded-grpo
