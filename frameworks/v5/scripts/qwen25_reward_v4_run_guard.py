"""Read-only preflight / locked reservation for the Qwen2.5-Omni-3B V4 ten-round base launcher."""
...
def run_contract(config: dict, mode: str, rounds: int) -> dict:
    ...
    if (not distributed.get("enabled") or distributed["world_size"] != 8
        ...
        raise ValueError("RewardV4 Qwen2.5-Omni-3B requires 8-rank NCCL")
    ...
    files += [PROJECT / "scripts/qwen2_5_omni_3b_captioner_candidate.py",
              PROJECT / "scripts/qwen25_reward_v4_run_guard.py",
              PROJECT / "scripts/run_qwen25_v4_10rounds.sh",
