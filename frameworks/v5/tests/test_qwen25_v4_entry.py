...
from scripts.qwen25_reward_v4_run_guard import run_contract,check_run_directory,MARKER

@pytest.fixture
def cfg(monkeypatch):
    for name in (*ENV_OVERRIDES,*INTEGER_ENV_OVERRIDES): monkeypatch.delenv(name,raising=False)
    return load_config('configs/train_8gpu_h100_qwen2_5_omni_3b_reward_v4.yaml')
...
def test_qwen_config_identity_and_shared_v4_rules(cfg):
    ...
    assert 'scripts/run_qwen25_v4_10rounds.sh' in contract['code_sha256']
